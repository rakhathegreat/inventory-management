...
export function ScrollShadowWrapper({
	...
	return (
		<div
			className={cn(
				"rounded-lg border overflow-hidden relative flex-1 min-h-0",
				className,
			)}>
			<div
				ref={scrollRef}
			...
			{canScrollTop && (
				<div
					className="absolute left-0 right-0 h-6 bg-linear-to-b from-background to-transparent pointer-events-none z-30"
					style={{ top: `${headerHeight}px` }}
				/>
			)}
			{canScrollBottom && (
				<div className="absolute bottom-0 left-0 right-0 h-8 bg-linear-to-t from-background to-transparent pointer-events-none rounded-b-lg z-30" />
			)}
		</div>
	);
}
