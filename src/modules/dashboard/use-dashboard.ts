...
const mapRequestSummary = (value: unknown): RequestSummary => {
	...
	const requestItems = Array.isArray(request.requestItems)
		? request.requestItems
		: [];

	return {
		id: readFirstText(request.id),
		requestNumber: readFirstText(
		...
		partnerCategory: readFirstText(
			request.partnerCategory,
			requesterProfile.partnerType,
		),
		status: readFirstText(request.status, "MENUNGGU"),
		requestedAt: readFirstText(
			request.requestedAt,
			request.createdAt,
			request.updatedAt,
		),
		itemsCount:
			typeof request.itemsCount === "number"
				? request.itemsCount
				: requestItems.reduce(
						(total, item) => total + readNumber(asRecord(item).quantity),
						0,
					),
	};
};
...
export function useDashboard() {
	...
	const [mitraDistribution, setMitraDistribution] = useState<
		{ mitra: string; tersedia: number; terpakai: number; total: number }[]
	>([]);
	const [mitraPerformanceMetrics, setMitraPerformanceMetrics] = useState<
		MitraPerformanceMetrics[]
	>([]);
	...
	const fetchDashboardData = useCallback(async () => {
		...
		try {
			const [transactionData, itemData, requestData, performanceData] =
				await Promise.all([
			...
			const mitraMap = new Map<
				string,
				{ tersedia: number; terpakai: number }
			>();
			...
			setInventoryStats({
				...
				tersedia: visibleItems.filter(
					(item: any) => item.status.trim().toLowerCase() === "tersedia",
				).length,
				diluar: visibleItems.filter((item: any) =>
					["diluar", "terdistribusi", "keluar", "digunakan"].includes(
						item.status.trim().toLowerCase(),
					),
				).length,
				rusak: visibleItems.filter(
					(item: any) => item.status.trim().toLowerCase() === "rusak",
				).length,
				hilang: visibleItems.filter(
					(item: any) => item.status.trim().toLowerCase() === "hilang",
				).length,
			});
	...
	const requestCounts = {
		menunggu: allRequests.filter((r) => normalizeKey(r.status) === "menunggu")
			.length,
	...
	const transactionSeries = useMemo(() => {
		...
		if (!chartTransactions.length) {
			...
			for (let day = 0; day < rangeDays; day += 1) {
				points.push({
					date: toDateKey(addDays(startDate, day)),
					masuk: 0,
					keluar: 0,
				});
			}
		...
		const points = new Map<
			string,
			{ date: string; masuk: number; keluar: number }
		>();
		for (let day = 0; day < rangeDays; day += 1) {
		...
		for (const transaction of chartTransactions) {
			if (
				...
				transaction.mitra?.trim().toLowerCase() !==
					selectedMitra.trim().toLowerCase()
			) {
				continue;
			}
