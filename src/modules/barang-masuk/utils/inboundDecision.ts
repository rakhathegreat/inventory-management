...
export function decideInboundScan(ctx: InboundScanContext): ScanDecision {
	...
	if (allowedIdentifiers.length > 0) {
		...
		const matched = allowedIdentifiers.find((prefix) =>
			upperKode.startsWith(prefix),
		);
		if (!matched) {
			return {
				action: "reject",
	...
	if (isDuplicate) {
		return {
			action: "reject",
			message: "Serial number sudah ada di sesi ini.",
	...
	if (!existingItem && !ctx.tipeBarang) {
		return {
			...
			description:
				"SN ini belum terdaftar di database. Pilih Model terlebih dahulu.",
		};
	}
	...
	if (ctx.kondisi === "baru" && existingItem) {
		return {
			...
			description:
				"SN ini sudah terdaftar. Silakan ubah kondisi ke 'dismantle' atau 'rusak'.",
		};
	}
	...
	const itemBrand =
		existingItem?.merek ||
		ctx.brand ||
		selectedModelInfo?.brand?.nama ||
		selectedModelInfo?.brand?.name ||
		detectBrandFromCode(trimmedKode, ctx.dbBrands) ||
		"";

	const isExistingCondition =
		ctx.kondisi === "dismantle" || ctx.kondisi === "rusak";
	const itemCategory = isExistingCondition
		? existingItem?.kategori || "ONT"
		: selectedModelInfo?.materialCategory?.nama ||
			existingItem?.kategori ||
			"ONT";

	let recommendedLocation = getRecommendedLocation(
		itemBrand,
		ctx.kondisi,
		itemCategory,
		ctx.dbLocations,
	...
	if (
		...
		if (
			recommendedLocation &&
			recommendedLocation.trim().toLowerCase() ===
				(existingItem.lokasiPenyimpanan || "").trim().toLowerCase()
		) {
	...
	const item: BarangMasukItem = {
		id: ctx.idSeed,
		nomor: trimmedKode,
		merek: isExistingCondition
			? existingItem?.merek || itemBrand || ""
			: itemBrand || "",
...
export function toIntakePayload(item: BarangMasukItem, todayISO: string) {
	return {
		...
		kondisi:
			item.status === "Rusak" ? "Rusak" : item.nomorPA ? "Dismantle" : "Baru",
...
const REASON_MESSAGES: Record<string, string> = {
	...
	SN_REGISTERED:
		"Serial number sudah terdaftar — gunakan kondisi Dismantle atau Rusak.",
	INVALID_SN_FOR_DISMANTLE:
		"SN tidak ditemukan — daftarkan sebagai material Baru terlebih dahulu.",
	...
	INVALID_MITRA_SOURCE:
		"Material belum bisa diterima — harus sudah keluar dari KP.",
